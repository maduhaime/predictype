import { ArrayIndexComparisonEnum, ArrayIndexComparisonOper } from './enums';
...
export function arrayIndexComparison<T>(
  ...
  const operators: Record<ArrayIndexComparisonEnum, (arr: T[], idx: number, val: T) => boolean> = {
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_EQUALS]: (arr, i, val) => arr[i] === val,
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_GREATER_THAN_OR_EQUALS]: (arr, i, val) => arr[i] >= val,
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_GREATER_THAN]: (arr, i, val) => arr[i] > val,
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_LESS_THAN_OR_EQUALS]: (arr, i, val) => arr[i] <= val,
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_LESS_THAN]: (arr, i, val) => arr[i] < val,
    [ArrayIndexComparisonEnum.VALUE_AT_INDEX_NOT_EQUALS]: (arr, i, val) => arr[i] !== val,
  };
