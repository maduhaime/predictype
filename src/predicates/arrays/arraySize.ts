...
export function arraySize(source: any[], oper: ArraySizeOper, target: number): boolean {
  const operators: Record<ArraySizeEnum, (arr: any[], val: number) => boolean> = {
    [ArraySizeEnum.LENGTH_EQUALS]: (arr, val) => arr.length === val,
    [ArraySizeEnum.LENGTH_GREATER_THAN_OR_EQUALS]: (arr, val) => arr.length >= val,
    [ArraySizeEnum.LENGTH_GREATER_THAN]: (arr, val) => arr.length > val,
    [ArraySizeEnum.LENGTH_LESS_THAN_OR_EQUALS]: (arr, val) => arr.length <= val,
    [ArraySizeEnum.LENGTH_LESS_THAN]: (arr, val) => arr.length < val,
  };
