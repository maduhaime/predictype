...
describe('mapEntry', () => {
  it('should return true for contains_entry', () => {
    ...
    expect(mapEntry(m, 'contains_entry', entry)).toBe(true);
  });

  it('should return false for contains_entry with wrong value', () => {
    const m = new Map([[1, 'a']]);
    const entry = [1, 'b'] as [number, string];
    expect(mapEntry(m, 'contains_entry', entry)).toBe(false);
  });
